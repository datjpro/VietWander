...
export function ProvinceMap({ provinces = [], activeProvinceId = '' }) {
  ...
  const [selectedProvinceId, setSelectedProvinceId] = useState(activeProvinceId || decoratedProvinces[0]?.id || 'ha-noi');
  ...
  useEffect(() => {
    ...
    if (!decoratedProvinces.some((province) => province.id === selectedProvinceId)) {
      setSelectedProvinceId(decoratedProvinces[0]?.id || 'ha-noi');
    }
    ...
    <div className="map-card">
      <div className="map-story-panel">
        <div className="map-card-copy">
          ...
          <p className="muted-copy">
            Component lấy danh sách từ <code>/api/provinces</code>, tự bù metadata còn thiếu và dựng lại thành bản đồ Việt Nam
            theo phong cách minh họa với cụm vùng, đường hành trình và điểm nhấn từng tỉnh.
          </p>
        </div>
        ...
        <div className="map-legend">
          {regionStats.map((region) => (
            <button
              ...
              key={region.label}
              onClick={() => {
                const targetProvince = decoratedProvinces.find((province) => province.region === region.label);
                if (targetProvince) {
                  setSelectedProvinceId(targetProvince.id);
                }
              }}
        ...
        {focusedProvince ? (
          <article className="map-focus-card" style={{ '--province-accent': focusedProvince.themeColor }}>
            ...
            <div className="map-tag-list">
              {(focusedProvince.popularTags || []).map((tag) => (
      ...
      <div className="map-visual-shell">
        <div className="map-visual">
          <svg className="vietnam-map" viewBox={`0 0 ${viewBox.width} ${viewBox.height}`} role="img" aria-label="Bản đồ Việt Nam hoạt họa đầy đủ">
            <defs>
